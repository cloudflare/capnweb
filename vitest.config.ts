    ...
    projects: [
      // Node.js
      {
        test: {
          ...
          // JavaScript -- no need to run it on every runtime. The limits and signal tests are
          // likewise plain JavaScript, so Node coverage is sufficient.
          include: ['__tests__/index.test.ts', '__tests__/flow-control.test.ts',
                    '__tests__/limits.test.ts', '__tests__/signal.test.ts',
