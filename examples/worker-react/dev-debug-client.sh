...
cd "$REPO_ROOT"
if [[ "${FORCE_BUILD:-0}" == "1" || ! -f dist/index.js || ! -f packages/capnweb-validate/dist/plugins/vite.mjs ]]; then
...
compiler.paths = {
  ...
  "capnweb-validate": [path.join(repoRoot, "packages/capnweb-validate/dist/index.d.mts")],
  "capnweb-validate/internal": [path.join(repoRoot, "packages/capnweb-validate/dist/internal/runtime.d.mts")],
};
...
const capnwebValidate = (await import(
  pathToFileURL(path.resolve('$REPO_ROOT/packages/capnweb-validate/dist/plugins/vite.mjs')).href
)).default;

const debugAliases = {
  'capnweb-validate/internal': path.resolve('$REPO_ROOT/packages/capnweb-validate/dist/internal/runtime.mjs'),
  'capnweb-validate': path.resolve('$REPO_ROOT/packages/capnweb-validate/dist/index.mjs'),
...
export default {
  ...
  plugins: [
    {
      name: 'capnweb-validate-debug-alias',
      enforce: 'pre',
      resolveId(id) {
    ...
    {
      name: 'capnweb-validate-debug-runtime-imports',
      enforce: 'post',
      transform(code, id) {
        if (!id.startsWith('$CLIENT_DIR/') || !code.includes('capnweb')) return null;
        let next = code
          .replaceAll('"capnweb-validate/internal"', JSON.stringify('/@fs/' + debugAliases['capnweb-validate/internal']))
          .replaceAll("'capnweb-validate/internal'", JSON.stringify('/@fs/' + debugAliases['capnweb-validate/internal']))
          .replaceAll('"capnweb-validate"', JSON.stringify('/@fs/' + debugAliases['capnweb-validate']))
          .replaceAll("'capnweb-validate'", JSON.stringify('/@fs/' + debugAliases['capnweb-validate']))
