...
function getRpcValidator(
  ...
  while (typeof klass === "function") {
    ...
    if (!registered && generatedValidators) {
      // First lookup for this class: fall back to validators generated by
      // `capnweb typecheck gen`. Cache on the class so subsequent lookups
      // hit the WeakMap fast path.
      let byName = generatedValidators[klass.name];
      if (byName) {
        setRpcMethodValidators(klass, byName);
        registered = rpcValidators.get(klass);
      }
    }
    let validator = registered?.[methodName];
