...
function emitSpecsModule(classes: ExtractedClassSpec[], runtimeImport: string): string {
  ...
  let lines = [
    ...
    // Imported lazily through capnweb's main entry. Safe under the
    // capnweb -> _typecheck-validators -> capnweb cycle because the binding
    // is only read inside `__capnweb_makeValidationError` at call time.
    "import { RpcValidationError as __CapnwebRpcValidationError } from \"capnweb\";",
    "",
    ...
    "  let message = prefix + \": \" + where + \": expected \" + failure.expected + \", got \" + failure.actual;",
    "  return new __CapnwebRpcValidationError(message, failure);",
    "}",
    "",
