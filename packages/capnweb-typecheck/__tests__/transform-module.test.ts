...
beforeEach(() => {
  dir = mkdtempSync(join(tmpdir(), "capnweb-typecheck-tm-"));
...
async function loadValidator(code: string, binding: string): Promise<TestServiceValidator> {
  ...
  let prelude = code.match(/import \* as __rt from "capnweb-typecheck\/internal";\n([\s\S]*?)\n\s*import\s+/);
...
declare module "capnweb-typecheck" {
...
describe("transformModule: server boundary rewrite", () => {
  it("rewrites newWorkersRpcResponse and emits a validator for the target type", () => {
    ...
    let src = write("worker.ts", `
      import { newWorkersRpcResponse } from "capnweb-typecheck";
    ...
    expect(code).toContain("__rt.__newWorkersRpcResponseWithTypecheck(req, new Api()");
    // Runtime import injected.
    expect(code).toContain(`import * as __rt from "capnweb-typecheck/internal"`);
    // Server-side validator emitted for the Api shape.
    expect(code).toContain("const __capnweb_typecheck_Api_server");
...
describe("transformModule: client session rewrite", () => {
  it("rewrites newHttpBatchRpcSession using the explicit type argument", () => {
    ...
    expect(code).toMatch(/__rt\.__newHttpBatchRpcSessionWithTypecheck<Api>\("\/rpc"/);
    expect(code).toContain("const __capnweb_typecheck_Api_client");
  ...
  it("rewrites `new RpcSession<T>(transport)` into a call helper", () => {
    ...
    let src = write("session.ts", `
      import { RpcSession, RpcTransport } from "capnweb-typecheck";
      import { RpcTarget } from "capnweb";
      interface Api extends RpcTarget {
    ...
    expect(code).toMatch(/__rt\.__newRpcSessionWithTypecheck<Api>\(t,\s*__capnweb_typecheck_Api_client\)/);
...
describe("transformModule: marker helper coverage", () => {
  let serverCases: Array<[string, string, string]> = [
    ["newWorkersRpcResponse", "return newWorkersRpcResponse(req, new Api());", "__newWorkersRpcResponseWithTypecheck"],
    ["newWorkersWebSocketRpcResponse", "return newWorkersWebSocketRpcResponse(req, new Api(), { tag: true });", "__newWorkersWebSocketRpcResponseWithTypecheck"],
    ["newHttpBatchRpcResponse", "return newHttpBatchRpcResponse(req, new Api(), { tag: true });", "__newHttpBatchRpcResponseWithTypecheck"],
    ["nodeHttpBatchRpcResponse", "return nodeHttpBatchRpcResponse(req, res, new Api(), { tag: true });", "__nodeHttpBatchRpcResponseWithTypecheck"],
  ];
  ...
  for (let [marker, call, helper] of serverCases) {
    it(`rewrites ${marker}`, () => {
      ...
      let src = write(`${marker}.ts`, `
        import { ${marker} } from "capnweb-typecheck";
        import { RpcTarget } from "capnweb";
        class Api extends RpcTarget {
      ...
      expect(code).toContain("__capnweb_typecheck_Api_server");
      expect(code).toMatch(/ping[^}]*args:\s*\[\s*__rt\.v\.string\s*\]/s);
    });
  }

  let clientCases: Array<[string, string, string]> = [
    ["newHttpBatchRpcSession", "return newHttpBatchRpcSession<Api>(\"/rpc\", { tag: true });", "__newHttpBatchRpcSessionWithTypecheck"],
    ["newWebSocketRpcSession", "return newWebSocketRpcSession<Api>(ws, localMain, { tag: true });", "__newWebSocketRpcSessionWithTypecheck"],
    ["newMessagePortRpcSession", "return newMessagePortRpcSession<Api>(port, localMain, { tag: true });", "__newMessagePortRpcSessionWithTypecheck"],
    ["RpcSession", "return new RpcSession<Api>(transport, localMain, { tag: true });", "__newRpcSessionWithTypecheck"],
  ];
  ...
  for (let [marker, call, helper] of clientCases) {
    it(`rewrites ${marker}`, () => {
      ...
      let src = write(`${marker}.ts`, `
        import { ${importName} } from "capnweb-typecheck";
        import { RpcTarget } from "capnweb";
        interface Api extends RpcTarget {
          ping(value: string): Promise<string>;
        }
      ...
      expect(code).toContain("__capnweb_typecheck_Api_client");
...
describe("transformModule: primitive matrix", () => {
  it("emits runtime validators for primitive and permissive leaves", async () => {
    ...
    let validator = await loadValidator(code, "__capnweb_typecheck_Api_server");
...
describe("transformModule: resolved TypeScript shapes", () => {
  it("lowers Omit, Pick, Partial, intersections, and aliases", async () => {
    ...
    let { code } = transform(src);
    let binding = code.match(/const (__capnweb_typecheck_[A-Za-z0-9_$]+_client) =/)?.[1];
...
describe("transformModule: rejected built-ins", () => {
  ...
  it("allows a method to opt out with @skipRpcValidation", async () => {
    ...
    let src = write("skip-validation.ts", `
      import { newWorkersRpcResponse, skipRpcValidation } from "capnweb-typecheck";
      import { RpcTarget } from "capnweb";
      class Api extends RpcTarget {
        @skipRpcValidation
    ...
    try {
      ...
      let client = rt.__newMessagePortRpcSessionWithTypecheck<any>(channel.port2, validator as never);
...
describe("transformModule: recursive types", () => {
  it("emits lazy validators for direct object recursion", () => {
    ...
    expect(code).toMatch(/let __capnweb_typecheck_Api_server_shape_\d+;/);
    expect(code).toContain("__rt.v.lazy(() => __capnweb_typecheck_Api_server_shape_");
    expect(code).toMatch(/getNode[^}]*returns:\s*__capnweb_typecheck_Api_server_shape_\d+/s);
  });
  ...
  it("emits lazy validators for recursive union aliases", () => {
    ...
    expect(code).toMatch(/__capnweb_typecheck_Api_server_shape_\d+ = __rt\.v\.union\(\[/);
    expect(code).toContain("__rt.v.array(__rt.v.lazy(() => __capnweb_typecheck_Api_server_shape_");
    expect(code).toMatch(/getJson[^}]*returns:\s*__capnweb_typecheck_Api_server_shape_\d+/s);
  });
...
describe("transformModule: dedup", () => {
  it("emits a single validator when one service is referenced twice", () => {
    ...
    let { code } = transform(src);
    let matches = code.match(/const __capnweb_typecheck_Api_server/g) ?? [];
    expect(matches.length).toBe(1);
    // Both call sites point at the same binding.
    let rewrites = code.match(/__rt\.__newWorkersRpcResponseWithTypecheck\(/g) ?? [];
  ...
  it("suffixes validators for same-name service collisions", () => {
    ...
    expect(code).toContain("const __capnweb_typecheck_Api_server =");
    expect(code).toContain("const __capnweb_typecheck_Api_server_2 =");
    expect(code).toMatch(/new One\.Api\(\), __capnweb_typecheck_Api_server\)/);
    expect(code).toMatch(/new Two\.Api\(\), __capnweb_typecheck_Api_server_2\)/);
...
describe("transformModule: runtime behavior", () => {
  ...
  it("emitted server validator rejects mistyped args", async () => {
    ...
    // as `const __capnweb_typecheck_Api_server = { ... };` - parse it out.
    let m = code.match(/const __capnweb_typecheck_Api_server = (\{[\s\S]*?\n\});/);
  ...
  it("client wrapper rejects bad outgoing args before transport", async () => {
    ...
    try {
      ...
      let client = rt.__newMessagePortRpcSessionWithTypecheck<{
  ...
  it("client wrapper rejects methods missing from the validator", async () => {
    ...
    try {
      ...
      let client = rt.__newMessagePortRpcSessionWithTypecheck<any>(channel.port2, validator);
  ...
  it("emitted recursive validator follows lazy back-references", async () => {
    ...
    expect(prelude, "validator prelude not found in:\n" + code).not.toBeNull();
    // eslint-disable-next-line @typescript-eslint/no-implied-eval
    let validator = new Function("__rt", `${prelude![1]!}\nreturn __capnweb_typecheck_Api_server;`)(rt) as {
