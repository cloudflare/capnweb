...
import { capnwebTypecheck } from "../src/plugin.js";
...
describe("marker APIs throw before the transform runs", () => {
  ...
  for (let [name, marker] of markers) {
    it(`${name} throws with a configure-the-plugin message`, () => {
      expect(() => marker()).toThrow(/capnweb-typecheck marker API was called before it was transformed/);
    });
  }
});
...
describe("TransformContext stub", () => {
  ...
  it("listSourceFiles yields the Program's source files", () => {
    ...
    let dir = mkdtempSync(join(tmpdir(), "capnweb-typecheck-ctx-"));
    try {
...
describe("unplugin adapters", () => {
  ...
  for (let name of ["vite", "rollup", "webpack", "rspack", "esbuild", "farm"] as const) {
    it(`exposes a ${name} adapter`, () => {
      let factory = (capnwebTypecheck as Record<string, unknown>)[name];
...
describe("CLI bin", () => {
  ...
  let repoBin = join(process.cwd(), "packages/capnweb-typecheck/dist/cli.cjs");
  ...
  it.skipIf(!existsSync(bin))("runs the published CJS help path", () => {
    ...
    expect(result.stdout).toContain("capnweb-typecheck build --out <dir>");
  });
});
...
describe("runBuild orchestration", () => {
  it("returns zero counts when the source set is empty", async () => {
    ...
    let src = mkdtempSync(join(tmpdir(), "capnweb-typecheck-src-"));
    let out = mkdtempSync(join(tmpdir(), "capnweb-typecheck-out-"));
    try {
  ...
  it("does not recurse into an output tree matched by tsconfig", async () => {
    ...
    try {
      ...
      let options = {
        ...
        out: ".wrangler/typecheck",
      };
      await expect(runBuild(options)).resolves.toEqual({ transformed: 0, copied: 1 });
      await expect(runBuild(options)).resolves.toEqual({ transformed: 0, copied: 1 });

      expect(existsSync(join(src, ".wrangler/typecheck/a.ts"))).toBe(true);
      expect(existsSync(join(src, ".wrangler/typecheck/.wrangler/typecheck/a.ts"))).toBe(false);
    } finally {
      rmSync(src, { recursive: true, force: true });
    }
  });
