# Testing `capnweb-typecheck` by hand

This package is a build-time transform. Users import marker APIs from
`capnweb-typecheck`; the bundler plugin or CLI rewrites those call sites and
...
```sh
npm run -w capnweb-typecheck build
npm run -w capnweb-typecheck test
```
...
This produces `packages/capnweb-typecheck/dist/cli.cjs`, which is what `npm`
links as the `capnweb-typecheck` bin.
...
```sh
node packages/capnweb-typecheck/dist/cli.cjs build \
  --cwd /tmp/cwt-demo \
  --tsconfig /tmp/cwt-demo/tsconfig.json \
  --out /tmp/cwt-demo/out
```

You should see `capnweb-typecheck: 1 transformed, 0 copied -> /tmp/cwt-demo/out`.
...
```ts
import * as __rt from "capnweb-typecheck/internal";
...
export function handler(req: Request): Response {
  return __rt.__newWorkersRpcResponseWithTypecheck(req, new Api(), __capnweb_typecheck_Api_server);
}
```
...
```txt
capnweb-typecheck: /tmp/cwt-demo/src/worker.ts:5:10: capnweb-typecheck:
...
`__rt.__newHttpBatchRpcSessionWithTypecheck<Api>("/rpc", __capnweb_typecheck_Api_client)`.
...
```
capnweb-typecheck: /tmp/cwt-demo/src/worker.ts:10:10: capnweb-typecheck:
...
- **"Module is in the program but no rewrite happens"** — the transform
  ...
  `"capnweb-typecheck"`. Confirm your import string is exact.
