...
// Internal runtime helpers called by transformed user code. Not part of the
// public API surface - the transform inserts imports from
// "capnweb-typecheck/internal" automatically and user source never references
// these names. Anything in this file is implementation detail that may change
...
const VALIDATOR_SHAPE = Symbol("capnweb-typecheck.validatorShape");
...
export type MethodSpec =
  | { unchecked: true }
  | { args: Validator[]; rest?: Validator; returns: Validator; unchecked?: false };
...
function fail(path: PropertyPath, expected: string, value: unknown): never {
  ...
  throw new RpcValidationError(
    `capnweb-typecheck: at ${formatPath(path)}: expected ${expected}, got ${actual}`,
...
function validateStubBrand(
    value: unknown, path: PropertyPath, options?: ValidationOptions): void {
 ...
 */
export const v = {
  string(value: unknown, path: PropertyPath, options?: ValidationOptions): void {
    if (isDeferredValidationValue(value, options)) return;
    if (typeof value !== "string") fail(path, "string", value);
  },
  number(value: unknown, path: PropertyPath, options?: ValidationOptions): void {
    if (isDeferredValidationValue(value, options)) return;
    if (typeof value !== "number") fail(path, "number", value);
  },
  boolean(value: unknown, path: PropertyPath, options?: ValidationOptions): void {
    if (isDeferredValidationValue(value, options)) return;
    if (typeof value !== "boolean") fail(path, "boolean", value);
  },
  bigint(value: unknown, path: PropertyPath, options?: ValidationOptions): void {
    if (isDeferredValidationValue(value, options)) return;
  ...
  undefined_(value: unknown, path: PropertyPath, options?: ValidationOptions): void {
    if (isDeferredValidationValue(value, options)) return;
  ...
  array(elem: Validator): Validator {
    return withShape((value, path, options) => {
      if (isDeferredValidationValue(value, options)) return;
  ...
  tuple(elements: Validator[]): Validator {
    return withShape((value, path, options) => {
      ...
      if (value.length !== elements.length) fail(path, `tuple(${elements.length})`, value);
      for (let i = 0; i < elements.length; i++) {
        elements[i]!(value[i], [...path, i], options);
      }
    }, { kind: "tuple", elements });
  },
  object(shape: Record<string, Validator>, name?: string, index?: Validator): Validator {
  ...
  literal(expected: string | number | boolean | null, label?: string): Validator {
...
function isDeferredValidationValue(
    value: unknown, options: ValidationOptions | undefined): boolean {
  ...
  return options?.allowDeferred === true && isRpcPromise(value);
}
...
// into
//   __newWorkersRpcResponseWithTypecheck(req, new Api(env), __validator)
...
function isUncheckedMethod(methodSpec: MethodSpec): methodSpec is { unchecked: true } {
  return methodSpec.unchecked === true;
}

function validateArgs(
    args: unknown[], methodSpec: Exclude<MethodSpec, { unchecked: true }>,
  ...
  if (methodSpec.rest) {
    ...
    fail([serviceName, prop, methodSpec.args.length],
      "no extra argument", args[methodSpec.args.length]);
  }
}
...
function canPassThroughMethod(prop: string): boolean {
  return PASSTHROUGH_METHODS.has(prop);
}

function missingMethod(serviceName: string, prop: string): never {
  throw new RpcValidationError(
    `capnweb-typecheck: ${serviceName}.${prop} is not in the generated validator`,
    {
...
function wrapServerTarget<T extends object>(target: T, validator: ServiceValidator): T {
  return new Proxy(target, {
    get(t, prop, receiver) {
      ...
      if (!methodSpec) {
        if (canPassThroughMethod(prop)) return orig.bind(t);
      ...
      return function wrapped(this: unknown, ...args: unknown[]): unknown {
        ...
        let result = Reflect.apply(orig as (...a: unknown[]) => unknown, t, args);
        return validateReturn(result, methodSpec.returns, [
...
function validateReturn(
    result: unknown, returns: Validator, path: PropertyPath,
    side: WrapSide): unknown {
  if (isRpcPromise(result)) {
...
function validateResolvedValue(
    value: unknown, validator: Validator, path: PropertyPath,
    side: WrapSide): unknown {
...
function wrapResolvedValue(
  ...
  if (shape.kind === "lazy") return wrapResolvedValue(value, shape.thunk(), path, side);
  if (shape.kind === "union") {
  ...
  if (shape.kind === "stub") {
    ...
    let valueType = typeof value;
    if (value === null || (valueType !== "object" && valueType !== "function")) return value;
    return side === "server"
  ...
  if (shape.kind === "array" || shape.kind === "tuple") {
    ...
    let validators = shape.kind === "array"
      ? value.map(() => shape.element)
      : shape.elements;
    let next: unknown[] | undefined;
    for (let i = 0; i < validators.length && i < value.length; i++) {
      let wrapped = wrapResolvedValue(value[i], validators[i]!, [...path, i], side);
      if (wrapped !== value[i]) {
        next ??= value.slice();
        next[i] = wrapped;
      }
  ...
  if (shape.kind === "object") {
    ...
    for (let [key, propValidator] of Object.entries(shape.properties)) {
      let wrapped = wrapResolvedValue(rec[key], propValidator, [...path, key], side);
      if (wrapped !== rec[key]) {
        next ??= { ...rec };
        next[key] = wrapped;
      }
...
function isRpcPromise(value: unknown): value is (...args: unknown[]) => unknown {
  let ctor = capnweb.RpcPromise as unknown;
...
function wrapRpcPromise(
    ...
    returns: Validator, path: PropertyPath, side: WrapSide): object {
  return new Proxy(result, {
    get(target, prop, receiver) {
      if (prop === "then") {
        return (onfulfilled?: ((value: unknown) => unknown) | null,
            onrejected?: ((reason: unknown) => unknown) | null) => {
          return callValidatedThen(target, receiver, returns, path, side,
      ...
      if (prop === "finally") {
        return (onfinally?: (() => unknown) | null) => {
          return callValidatedThen(target, receiver, returns, path, side, (value) => {
      ...
      if (methodSpec) {
        return function wrappedPipelined(this: unknown, ...args: unknown[]): unknown {
          let orig = Reflect.get(target, prop, receiver);
          if (isUncheckedMethod(methodSpec)) {
            return Reflect.apply(orig as (...a: unknown[]) => unknown, target, args);
          }
          validateArgs(args, methodSpec, serviceNameFor(returns) ?? "Service", prop, {
            allowDeferred: side === "client",
          });
          let result = Reflect.apply(orig as (...a: unknown[]) => unknown, target, args);
          return validateReturn(result, methodSpec.returns, [...path, prop, "<return>"], side);
        };
      }
      let propValidator = propertyValidatorFor(returns, prop);
      if (propValidator) {
        let nextPath = numericKey(prop) ? [...path, Number(prop)] : [...path, prop];
        let next = Reflect.get(target, prop, receiver);
        if (isRpcPromise(next)) return wrapRpcPromise(next, propValidator, nextPath, side);
        return validateResolvedValue(next, propValidator, nextPath, side);
      }
      if (canPassThroughMethod(prop)) return Reflect.get(target, prop, receiver);
...
function callValidatedThen(
    target: (...args: unknown[]) => unknown, receiver: unknown,
    returns: Validator, path: PropertyPath, side: WrapSide,
  ...
  return Reflect.apply(then, target, [(value: unknown) => {
    let wrapped = validateResolvedValue(value, returns, path, side);
    return onfulfilled ? onfulfilled(wrapped) : wrapped;
  }, onrejected]);
}

function methodSpecFor(validator: Validator, prop: string): MethodSpec | undefined {
  let shape = shapeOf(validator);
...
function propertyValidatorFor(validator: Validator, prop: string): Validator | undefined {
  let shape = shapeOf(validator);
  ...
  if (shape?.kind === "tuple" && numericKey(prop)) return shape.elements[Number(prop)];
  return undefined;
}
...
function splitTrailingValidator(rest: unknown[]): {
  args: unknown[];
  validator: ServiceValidator;
} {
  let validator = rest.at(-1);
  if (!isServiceValidator(validator)) {
    throw new Error("capnweb-typecheck: internal helper missing validator");
  }
...
export function __newWorkersRpcResponseWithTypecheck(
    request: Request, localMain: object, validator: ServiceValidator,
): Promise<Response> {
  return capnweb.newWorkersRpcResponse(
    request, wrapServerTarget(localMain, validator));
}

export function __newHttpBatchRpcResponseWithTypecheck(
    request: Request, localMain: object,
    ...rest: [ServiceValidator] | [capnweb.RpcSessionOptions | undefined, ServiceValidator]
): Promise<Response> {
  ...
  return capnweb.newHttpBatchRpcResponse(
    request, wrapServerTarget(localMain, validator), options);
}

export function __newWorkersWebSocketRpcResponseWithTypecheck(
    request: Request, localMain: object,
    ...rest: [ServiceValidator] | [capnweb.RpcSessionOptions | undefined, ServiceValidator]
): Response {
  let { args, validator } = splitTrailingValidator(rest);
...
export function __nodeHttpBatchRpcResponseWithTypecheck<
    Req extends { method?: string; url?: string; headers?: unknown },
    Res extends object,
>(
    request: Req, response: Res, localMain: object,
    ...rest: [ServiceValidator] | [capnweb.RpcSessionOptions | undefined, ServiceValidator]
): Promise<void> {
  let { args, validator } = splitTrailingValidator(rest);
  let [options] = args as [capnweb.RpcSessionOptions?];
  return (capnweb.nodeHttpBatchRpcResponse as unknown as (
      req: Req, res: Res, target: unknown, opts?: capnweb.RpcSessionOptions,
    ) => Promise<void>)(
...
//   __newHttpBatchRpcSessionWithTypecheck(url, __validator)
...
function wrapClientStub(stub: object, validator: ServiceValidator): object {
  return new Proxy(stub, {
    get(t, prop, receiver) {
      ...
      let methodSpec = validator.methods[prop];
      if (!methodSpec) {
        if (canPassThroughMethod(prop)) return (orig as (...a: unknown[]) => unknown).bind(t);
        return missingMethod(validator.serviceName, prop);
      }
      return function wrapped(this: unknown, ...args: unknown[]): unknown {
        if (isUncheckedMethod(methodSpec)) {
...
export function __newHttpBatchRpcSessionWithTypecheck<T>(
...
export function __newWebSocketRpcSessionWithTypecheck<T>(
    webSocket: WebSocket | string,
    ...rest: [ServiceValidator]
...
export function __newMessagePortRpcSessionWithTypecheck<T>(
    port: MessagePort,
    ...rest: [ServiceValidator]
      | [unknown, ServiceValidator]
      | [unknown, capnweb.RpcSessionOptions | undefined, ServiceValidator]
...
// ---------------------------------------------------------------------------
// `RpcSession` constructor wrapping. The transform rewrites
//   new RpcSession<Api>(transport)
// into
//   __rt.__newRpcSessionWithTypecheck(transport, __validator)
// The constructor returns a session, not the remote main, so the helper has
// to return something callers can still drive (`.getRemoteMain()`, `.drain()`,
// `.getStats()`). We hand back a `Proxy` that intercepts only `getRemoteMain`
// to wrap its return with the client validator and otherwise forwards.
// ---------------------------------------------------------------------------

export function __newRpcSessionWithTypecheck<T extends capnweb.RpcCompatible<T>>(
  ...
  let session = new capnweb.RpcSession<T>(
      transport, localMain as undefined, options);
  return new Proxy(session, {
    get(target, prop, receiver) {
      ...
      // Bind through so `this` stays the original session for methods like
      // `drain()` / `getStats()` that read private state.
