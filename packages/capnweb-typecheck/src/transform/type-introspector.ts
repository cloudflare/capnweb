...
function hasSkipRpcValidationDecorator(ctx: ResolveContext, decl: ts.Declaration): boolean {
  ...
  for (let decorator of tsm.getDecorators?.(decl) ?? []) {
    ...
    if (sym?.getName() === "skipRpcValidation" && isCapnwebTypecheckSymbol(sym)) {
      return true;
    }
  }
  return false;
...
function isCapnwebTypecheckSymbol(sym: ts.Symbol): boolean {
  for (let decl of sym.getDeclarations() ?? []) {
    let fileName = decl.getSourceFile().fileName;
    if (/[\\/]capnweb-typecheck[\\/]/.test(fileName)) return true;
  }
  let parent = (sym as ts.Symbol & { parent?: ts.Symbol }).parent;
  return parent !== undefined && (parent.escapedName as string) === '"capnweb-typecheck"';
}
