...
// Per-module transform for marker imports from "capnweb-typecheck". It
...
function assertValidTextEdits(code: string, ordered: TextEdit[]): void {
  for (let edit of ordered) {
    if (edit.start < 0 || edit.end < edit.start || edit.end > code.length) {
      throw new Error(`capnweb-typecheck: invalid text edit [${edit.start},${edit.end})`);
    }
  }
  for (let i = 1; i < ordered.length; i++) {
    ...
    if (left.start === right.start && left.end === right.end) {
      throw new Error(`capnweb-typecheck: duplicate text edit at ${left.start}`);
    }
    if (left.end > right.start) {
      throw new Error(
        `capnweb-typecheck: overlapping text edits ` +
...
const MARKERS: Record<string, {
  ...
  newWorkersRpcResponse:           { side: "server", form: "call", helper: "__newWorkersRpcResponseWithTypecheck",           targetArgIndex: 1 },
  newWorkersWebSocketRpcResponse:  { side: "server", form: "call", helper: "__newWorkersWebSocketRpcResponseWithTypecheck",  targetArgIndex: 1 },
  newHttpBatchRpcResponse:         { side: "server", form: "call", helper: "__newHttpBatchRpcResponseWithTypecheck",         targetArgIndex: 1 },
  nodeHttpBatchRpcResponse:        { side: "server", form: "call", helper: "__nodeHttpBatchRpcResponseWithTypecheck",        targetArgIndex: 2 },
  // Client sessions: the remote service type is the explicit type argument
  // (or contextual). The call's return type is `RpcStub<T>`.
  newHttpBatchRpcSession:    { side: "client", form: "call", helper: "__newHttpBatchRpcSessionWithTypecheck",    targetArgIndex: -1 },
  newWebSocketRpcSession:    { side: "client", form: "call", helper: "__newWebSocketRpcSessionWithTypecheck",    targetArgIndex: -1 },
  newMessagePortRpcSession:  { side: "client", form: "call", helper: "__newMessagePortRpcSessionWithTypecheck",  targetArgIndex: -1 },
  ...
  RpcSession:                { side: "client", form: "new",  helper: "__newRpcSessionWithTypecheck",              targetArgIndex: -1 },
};

const PACKAGE_NAME = "capnweb-typecheck";
      ...
      if (binding) {
        ...
        if (shape === null) {
          ...
          throw buildError(sf, call,
            `capnweb-typecheck: could not resolve a concrete service type for ` +
        ...
        if (bad.length > 0) {
          ...
          throw buildError(sf, call,
            `capnweb-typecheck: \`${binding.localName}(...)\` references types ` +
