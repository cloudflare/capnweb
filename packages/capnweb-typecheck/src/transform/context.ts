...
export function createTransformContext(
  ...
  function ensureProgram(): ts.Program {
    ...
    if (!tsconfigPath) {
      throw new Error(
        `capnweb-typecheck: tsconfig.json not found (cwd=${cwd}). ` +
        `Pass \`tsconfig\` to capnwebTypecheck({...}) or run from a directory ` +
        `with a tsconfig.`,
      );
    }
  ...
  function readParsedConfig(tsconfigPath: string): ts.ParsedCommandLine {
    ...
    if (raw.error) {
      throw new Error(
        `capnweb-typecheck: failed to read ${tsconfigPath}: ` +
