...
## What a disconnect actually destroys
...
addressable — a [Durable Object](/servers/workers/), a database, a queue. The demo's point is only
that it must not live *in the session*.
:::
...
Three things worth noticing in that handful of lines.
...
`[Symbol.dispose]()`, which also runs when the session dies — which is what stops the timer on an
abrupt disconnect.
...
replay is a denial-of-service vector wearing a helpful hat — see
...
What that gives up: the Worker's `fetch` handler is not involved, so the upgrade handling is the one
part of the example the page does not exercise. What it keeps: the API implementation, the session,
the wire protocol, calls in both directions, and a connection that can genuinely be severed — which
is the only thing this example is really about.
...
That version is a real WebSocket against a real Worker, so you can also disconnect it the honest
way, by turning off your network.
