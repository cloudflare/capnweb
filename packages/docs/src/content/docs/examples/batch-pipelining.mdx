...
counts are real but nothing crosses the network. Same code, same protocol — see
[how the playground works](#how-this-page-runs).
:::
...
The demo above is the example's real Worker and its real browser client, bundled together into one
...
The terminal client runs the same comparison against a real server over a real network:
...
- [RpcPromise & pipelining](/concepts/promises/) — how the promise references actually work.
