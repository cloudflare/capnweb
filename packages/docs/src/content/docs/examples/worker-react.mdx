...
`App.tsx` imports the `Api` class from `server/worker.ts` **as a type**. That gives the client full
...
That runs it as a real Worker over a real network, which is the honest way to feel the round trips.
