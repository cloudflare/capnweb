...
describe("bigint deserialization limits", () => {
  it("accepts a legacy decimal bigint at the default digit limit", () => {
  ...
  it("round-trips bigints through the hex wire format", () => {
    let values = [
      0n,
      255n,
      -255n,
    ...
    for (let value of values) {
      ...
      expect(wire).toMatch(/^\["bigint","-?0x[0-9a-f]+"\]$/);
      expect(deserialize(wire)).toBe(value);
    }
  });

  it("accepts legacy decimal bigint strings", () => {
    expect(deserialize('["bigint","123"]')).toBe(123n);
    expect(deserialize('["bigint","-123"]')).toBe(-123n);
  });

  it("accepts signed and unsigned hex bigint strings", () => {
    expect(deserialize('["bigint","0xff"]')).toBe(255n);
    expect(deserialize('["bigint","-0xff"]')).toBe(-255n);
    expect(deserialize('["bigint","0xFF"]')).toBe(255n);
  });

  it("rejects malformed bigint strings before calling BigInt()", () => {
    expect(() => deserialize('["bigint","0xg"]')).toThrowError(/invalid characters/);
    expect(() => deserialize('["bigint","0x"]')).toThrowError(/invalid characters/);
    expect(() => deserialize('["bigint","-0x"]')).toThrowError(/invalid characters/);
    expect(() => deserialize('["bigint","+0xff"]')).toThrowError(/invalid characters/);
  ...
  it("honors a per-session override of maxBigIntDigits", async () => {
    let underLimit = '["bigint","0xff"]';
    let overLimit = '["bigint","0xfff"]';

    // The cap is applied to the whole bigint string, regardless of radix.
